...
class Test extends authCommand_1.AuthCommand {
    ...
    async run() {
        ...
        const checks = Object.entries(project.data.check)
            ...
            return (0, test_filters_1.filterByTags)(targetTags?.map((tags) => tags.split(',')) ?? [], tags);
        })
            .map(([key, check]) => {
            check.logicalId = key;
        ...
        for (const check of checks) {
            ...
            if (!(check instanceof constructs_1.PlaywrightCheck) || check.codeBundlePath) {
                continue;
            }
            const { relativePlaywrightConfigPath, browsers, key, } = await constructs_1.PlaywrightCheck.bundleProject(check.playwrightConfigPath, check.include);
        ...
        const runner = new test_runner_1.default(config_1.default.getAccountId(), project, checks, location, timeout, verbose, shouldRecord, repoInfo, ciInfo.environment, updateSnapshots, configDirectory, testRetryStrategy);
